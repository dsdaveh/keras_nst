...
# J_content -- scalar that you compute using equation 1 above.
  ...
  J_content <- a_C_unrolled %>%
    ...
    tf$square() %>%
    tf$reduce_sum() / (4 * dims[2] * dims[3] * dims[4])
...
compute_layer_style_cost <- function(a_S, a_G) {
  ...
  # a_S <- tf$transpose(tf$reshape(a_S, c(dims[2] * dims[3], dims[4]))) 
  # a_G <- tf$transpose(tf$reshape(a_G, c(dims[2] * dims[3], dims[4]))) 
  ...
  J_style_layer <- GS %>% tf$subtract(GG) %>% tf$square() %>% tf$reduce_sum() / 
    (4 * dims[4]^2 * (dims[2] * dims[3])^2 )
  #tf.reduce_sum(tf.square(tf.subtract(GS, GG))) / (4 * n_C ### END CODE HERE ###
...
```
```{r}
STYLE_LAYERS <- data.frame( layer = paste0('conv', 1:5, '_1'), coeff = 0.2)
```
```{r}
compute_style_cost <- function(model, STYLE_LAYERS) {
